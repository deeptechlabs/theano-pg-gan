...
class Loader:
    def __init__(self,
        dataset = 'mscoco', # e.g. 'cifar10-32.h5'
        which_set='train',
        batch_size=5,
        seq_per_subset=0,
        seq_length=0,
        ...
        self.data_augm_kwargs = data_augm_kwargs
        ...
        self.trainiterator = self.set_trainiter()
        self.train_nsamples = trainiter.nsamples
        self.nclasses = trainiter.nclasses
        self.nbatches = trainiter.nbatches
        self.train_batch_size = trainiter.batch_size

        if self.verbose:
            print("Train %d" % (train_nsamples))

    def set_trainiter(self):
        if self.dataset == 'mscoco':
            self.trainiter = MSCocoDataset(
                which_set='train',
                batch_size=5,
                seq_per_subset=0,
                seq_length=0,
    ...
    def get_random_minibatch(self):
        train_group = self.trainiterator.next()
        # train_group checks
        #assert train_group[0].ndim == 4
        #assert train_group[0].shape[0] <= self.train_batch_size
        #assert train_group[0].shape[1:] == (72, 59, 3)
        #assert train_group[0].min() >= 0
        #assert train_group[0].max() <= 1
        #assert train_group[1].ndim == 4
        #assert train_group[1].shape[0] <= self.train_batch_size
        #assert train_group[1].shape[1:] == (72, 59, self.nclasses)
        return train_group
